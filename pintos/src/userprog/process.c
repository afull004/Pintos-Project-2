...
process_execute (const char *file_name) 
{
  ...
  //printf("from thread exec: current thread - %s   %s\n\n", thread_current()->name, file_name);
...
{
	//printf("entering wait %s\n\n", thread_current()->name);
	
	struct childProcess *cp = getChildProcess(child_tid);
...
//  struct thread *cur = thread_current ();
  ...
  struct thread *cur = thread_current();
  struct list_elem *next, *e = list_begin(&cur->files);
